...
<div class="container">
    ...
    <form method="post" action="vet_signin.php" id="registerForm" class="col-8 col-sm-8 col-md-6 col-lg-5 col-xl-4 mx-auto mt-4" >
        <div class="mb-3">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif (!existsVet($pdo, $email)) {
        $token = createToken(20);
        if ($token) {
            $id_vet = registerVet($pdo, $password, $firstname, $lastname , $phone,$email, $token);
            try {
                $body = "Your VET username is $email. To activate your account click on the <a href=" . SITE . "vet_active.php?token=$token>link</a>";
                sendEmail($pdo, $email, $emailMessages['vet_signIn'], $body, $id_vet);
                redirection("vet_signin.php?r=3");
            } catch (Exception $e) {
                error_log("****************************************");
                error_log($e->getMessage());
                error_log("file:" . $e->getFile() . " line:" . $e->getLine());
                redirection("vet_signin.php?r=11");
            }
        }
    } else {
        $r = 2;
    ...
    if ($r != 0) {
        redirection("vet_signin.php?r=$r");
    }
}
