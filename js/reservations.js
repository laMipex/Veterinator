document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelector('#saveDoneTreatment').addEventListener('click', function() {
       ...
       // let reservationCode = document.querySelector('input[type="hidden"]').value;  // Assuming you store the reservation code in a hidden input
...
function sendData(code, codeInput, negtivePoints) {
    ...
    if (code !== codeInput && negtivePoints === 0) {
        resultMessage.innerHTML = "Code is not valid!"
    ...
    if (isValid) {
                    ...
                    if (jsonData.reservations && jsonData.reservations.length > 0) {
                        ...
                        jsonData.reservations.forEach(reservation => {
                            const reservationInfo = `
                                <div>
                                    <p>User: ${reservation.vet_fname} ${reservation.vet_lname}</p>
                                    ...
                                    <input type="text" id="duration" value="${reservation.service_duration}">
        ...
        let data = JSON.stringify({"code_input": codeInput,"negative_points": parseInt(negtivePoints),"code":code });
        request.send(data);
    }
}
