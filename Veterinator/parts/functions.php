...
function registerUser(PDO $pdo, string $password, string $firstname, string $lastname,string $phone, string $email, string $token): int
{
    ...
    $sql = "INSERT INTO users(user_password,user_fname,user_lname,phone,user_email,registration_token, registration_expires,active)
...
function sendEmail(PDO $pdo, string $email, array $emailData, string $body, int $id_user): void
{
    ...
    try {
        ...
        //ovde stavis info za tvoj mailtrap

        $phpmailer->isSMTP();
        $phpmailer->Host = 'sandbox.smtp.mailtrap.io';
        $phpmailer->SMTPAuth = true;
        $phpmailer->Port = 2525;
        $phpmailer->Username = '61538825639521';
        $phpmailer->Password = '4885553087c123';

        //do ovog kopiras, i brises prvi red
