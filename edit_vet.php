...
/*if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old_password && $new_password) {
        $query = $pdo->prepare("SELECT vet_password FROM vet WHERE id_vet = :id");
        $query->execute(['id' => $id_vet]);
        $vet = $query->fetch();

        if (password_verify($old_password, $vet['vet_password'])) {
            $new_password_hashed = password_hash($new_password, PASSWORD_DEFAULT);
            $updatePasswordQuery = $pdo->prepare("UPDATE vet SET vet_password = :vet_password WHERE id_vet = :id");
