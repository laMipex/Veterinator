...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($negativePoints == 1) {
        ...
        $getIdSql = "SELECT u.id_user
                 ...
                 WHERE r.code = :codeInput";
        $getIdQuery = $pdo->prepare($getIdSql);
        $getIdQuery->bindParam(':codeInput', $code, PDO::PARAM_STR);
        $getIdQuery->execute();
        ...
        $updateReservationQuery->bindParam(':reservationCode', $code, PDO::PARAM_STR);
        ...
        $sql2 = "SELECT r.id_reservation, r.id_pet, p.pet_name, r.id_service, s.service_name, r.id_vet, r.reservation_date,
             r.reservation_time, r.treatment_price, r.code, r.service_duration, u.id_user, u.vet_fname, u.vet_lname
             FROM reservations r 
             INNER JOIN pet p ON p.id_pet = r.id_pet
             INNER JOIN users u ON p.id_user = u.id_user
